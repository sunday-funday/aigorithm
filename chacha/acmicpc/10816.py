...
n = int(input())
card_nums = sorted(map(int,input().split()))
...
for target in sorted(target_nums):
    count = 0
    if target not in dict:
        while idx < n:
            if target == card_nums[idx]:
                count += 1
                idx += 1
            elif target > card_nums[idx]:
                idx += 1
            else: break
        
        dict[target] = count

print(' ' .join(str(dict[i]) for i in target_nums))